...
pub fn encode_vec(
  ...
  use size <- result.try(list.length(items) |> u32)
  builder
  |> encode_u32(size)
  // this function is a loop over the items in the vector
  |> list.try_fold(items, _, encode_fn)
}
...
pub fn decode_vec(
  ...
  do_decode_vec(rest, size |> unwrap_u32, [], decode_fn)
}
...
fn do_decode_vec(
  ...
  case size {
    ...
    _ -> {
      ...
      do_decode_vec(rest, size - 1, acc |> list.append([val]), decode_fn)
    }
  }
}

...
pub fn encode_struct_type_test() {
  bytes_builder.new()
  |> types.encode_struct_type(structure_types.StructType(shine_tree.empty))
  |> should_equal_helper(<<0>>)
...
pub fn table_section_test() {
  ...
  |> modules.encode_table_section(
    structure_modules.TableSection(shine_tree.from_list([example_table])),
  )
  |> should_equal_helper(<<
    0x04,
    section_length:bits,
  ...
  bytes_builder.new()
  |> modules.encode_table_section(
    structure_modules.TableSection(shine_tree.from_list([example_table_init])),
  )
